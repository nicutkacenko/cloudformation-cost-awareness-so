...
def extract_tags(archives_dir, archives_dict, output_dir):
    ...
    tags = [t for t in helper.extract_tags(ap, xf, "cloudformation")]
    helper.export_json(output_dir+"cloudformation-tags.json", tags)
...
if __name__ == '__main__':
    ...
    # helper.print_xml_rows(output_dir+data_files["posts"]["archive_path"], data_files["posts"]["xml_filename"], 1)

    # STEP 1 - Extract question posts and associated data
    # question_filters = [
    #     ('PostTypeId', lambda x: x == '1'),
    #     ('Tags', lambda x: 'cloudformation' in x),
    # ]
